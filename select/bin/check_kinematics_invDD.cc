/**********************************
 *
 *	File Name : check_kinematics_DD.cc
 *	Date : 200225
 *	
 *	Description : If you want to change the kinematics
 *				 1. change hists' axis name
 *				 2. change hists' bin setting
 *				 3. change content code to fill in kinematic you want
 *				 4. change output file name (string kinematic)
 *
 *				 It's for CR1's Data-Driven
 *
 * ********************************/
...
int main(int argc,char* argv[])
{
	...
	get_lumi_weight( Weights_map , "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/full_sel/full_16_CR_inv.txt" );
	...
	get_path( Data_Set_Path , "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/full_sel/full_16_CR_inv.txt" );
...
*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				hists.FillHist( "2C", k, *channel, kk, 0., sel.Weight() );
		...
		ofstream f;
		f.open( "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/information/CR_inv_kinematics.txt",fstream::app );		
	...
	string new_file_name = string( "check_LJetEta_invCR_" ) + time_str + ".root";
	//string new_file_name = string("check_") + kine_name + string("_invCR_") + time_str + ".root";
