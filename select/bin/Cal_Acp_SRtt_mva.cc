...
*/
	...
	string new_file_name = training_name + string("_Acp_") + time_str + ".root";
	...
	ofstream f;
	f.open( "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/information/other.txt",fstream::app );		

	f << "exe name " << argv[0] << endl;
	f << "starting time is : " << startingtime << endl;
	f << "Finish time is : " << time_str << endl;
	f << "Total events # / pass sel events # : " << TOTAL <<  " / " << PASS << endl;
