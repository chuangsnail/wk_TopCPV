...
#include "TopCPViolation/select/interface/MVA_Algo/20var/TMVAClassification_MLP.class.C"
...
*/
			...
			{	sel.PlugInReweightTool(&bmgr, &pumgr, &lepsfmgr, &hltmgr);	}
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				lepton.SetPxPyPzE( leptonInfo.Px[sel.Getidx_lep()] ,leptonInfo.Py[sel.Getidx_lep()], leptonInfo.Pz[sel.Getidx_lep()], leptonInfo.Energy[sel.Getidx_lep()]);
