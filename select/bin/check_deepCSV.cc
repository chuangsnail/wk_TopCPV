...
int main(int argc,char* argv[])
{
	...
	//*** make histograms ***//
	Hists hists;
	hists.SetAxisInfo( " ;deepCSV value;Events[No.]", "" );
	hists.SetBinInfo( 100, 0., 1. );
	hists.NoCutModeON();
...
*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				vector<double> deepCSV_v;
				sel.SelJets_deepCSV( deepCSV_v, "all" );

				for(int i=0;i<(int)deepCSV_v.size();++i)
				{
					hists.FillHist( "NC", k, *channel, deepCSV_v.at(i), 0., sel.Weight() );
				}
	...
	string new_file_name = string("check_deepCSV_") + time_str + ".root";
