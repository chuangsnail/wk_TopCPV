...
*/
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				if( *channel == "mu" )
				{
					hists.h_l_mu.at(k)->Fill( Mlb, sel.Weight() );
					hists.h_mu.at(k)->Fill( Mjjb, sel.Weight() );
				}
				else if( *channel == "el" )
				{
					hists.h_l_el.at(k)->Fill( Mlb, sel.Weight() );
				...
				if( *channel == "mu" )
				{
					hists.h_l_mu_c.at(k)->Fill( Mlb, sel.Weight() );
					hists.h_mu_c.at(k)->Fill( Mjjb, sel.Weight() );
				}
				else if( *channel == "el" )
				{
					hists.h_l_el_c.at(k)->Fill( Mlb, sel.Weight() );
					hists.h_el_c.at(k)->Fill( Mjjb, sel.Weight() );
				}
				...
				if( *channel == "mu" )
				{
					hists.h_l_mu_cc.at(k)->Fill( Mlb, sel.Weight() );
					hists.h_mu_cc.at(k)->Fill( Mjjb, sel.Weight() );
				}
				else if( *channel == "el" )
				{
					hists.h_l_el_cc.at(k)->Fill( Mlb, sel.Weight() );
					hists.h_el_cc.at(k)->Fill( Mjjb, sel.Weight() );
				}
	...
	string new_file_name = string("SR_chi2_") + time_str + ".root";
