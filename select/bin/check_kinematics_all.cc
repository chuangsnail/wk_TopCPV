/**********************************
 *
 *	File Name : check_kinematics_SR.cc
 *	Date : 200225
 *	
 *
 *
...
int main(int argc,char* argv[])
{
	...
	get_path( Data_Set_Path , "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/full_sel/original.txt" );
	...
	File f_TT_001((char*)TT[0].c_str(),"bprimeKit/root");
	files_TT.push_back(f_TT_001);
	for(int i=0;i<(int)DY.size();i++){
		File f_DY((char*)DY[i].c_str(),"bprimeKit/root");			files_DY.push_back(f_DY);
	}
	for(int i=0;i<(int)WJets.size();i++){
		File f_WJets((char*)WJets[i].c_str(),"bprimeKit/root");		files_WJets.push_back(f_WJets);
	}
	for(int i=0;i<(int)VV.size();i++){
		File f_VV((char*)VV[i].c_str(),"bprimeKit/root");			files_VV.push_back(f_VV);
	}
	for(int i=0;i<(int)ST.size();i++){
		File f_ST((char*)ST[i].c_str(),"bprimeKit/root");			files_ST.push_back(f_ST);
	}	
	for(int i=0;i<(int)QCD.size();i++){
		File f_QCD((char*)QCD[i].c_str(),"bprimeKit/root");			files_QCD.push_back(f_QCD);
	}
	...
	File f_Data_1((char*)Data_SM[0].c_str(),"bprimeKit/root");
	for(int i=1;i<(int)Data_SM.size();i++){
		f_Data_1.AddFile((char*)Data_SM[i].c_str());
	}
	File f_Data_2((char*)Data_SE[0].c_str(),"bprimeKit/root");
	...
	//*** make histograms ***//
	Hists hists;
	hists.SetAxisInfo( " ;;Events[No.]", "" );
	hists.SetBinInfo( 10, -0.5, 9.5 );
	hists.NoCutModeON();
	hists.OneCutModeON();
	hists.TwoCutModeON();
	...
	for(int k=0;k<(int)files_map.size();k++)		//
	{
		...
		{	
			...
			set_entries += t_entries * lumi_weight;
			...
			for(int entry=0;entry<(int)t_entries;++entry)
			{
				...
				sel.SelJets();
				NJets = sel.seljets_No();
				sel.SelBJets();
				NBJets = sel.selbjets_No();
				NTightLeptons = sel.TightLeptonNo();
				
				hists.FillHist( "NC", k, "mu", (double)NJets, 0., lumi_weight );
				hists.FillHist( "1C", k, "mu", (double)NBJets, 0., lumi_weight );
				hists.FillHist( "2C", k, "mu", (double)NTightLeptons, 0., lumi_weight );
		...
		ofstream f;
		f.open( "/wk_cms2/cychuang/CMSSW_9_4_13/src/TopCPViolation/information/original.txt",fstream::app );		
		
		f << endl << setw(15) << left << "Sample" << right << ":" << setw(15) << right << Set_name << endl;
		f << "Events No before selection : " << set_entries << endl; 
	...
	string new_file_name = string("check_NJetsNBJetsNTLep_all_") + time_str + ".root";
